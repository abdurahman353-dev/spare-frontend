...
function ProductCard({
  ...
  const activeProduct = useMemo(() => {
    return (
      brandOptions.find((o) => o.id === selectedOptionId) ||
      brandOptions[0] || {
        id: product.id,
  ...
  const availableInventories =
    activeProduct.inventories?.filter((i: any) => i.quantity > 0) || [];
  const isInCart = cart.some(
    (item) => item.id === activeProduct.id && item.warehouse_id === selectedWarehouseId
  );
      ...
      >
        {activeProduct.status === "Inactive" ? (
          <div className="absolute top-3 left-3 z-10">
            <Badge className="bg-red-600 text-white border-none font-bold text-[10px] px-2 py-0.5">
              INACTIVE
            </Badge>
          </div>
        ) : availableInventories.length === 0 ? (
          <div className="absolute top-3 left-3 z-10">
            <Badge className="bg-zinc-800 text-white border-none font-bold text-[10px] px-2 py-0.5">
              OUT OF STOCK
            </Badge>
          </div>
        ) : availableInventories.reduce((acc: number, inv: any) => acc + inv.quantity, 0) <= 5 ? (
          <div className="absolute top-3 left-3 z-10">
            <Badge className="bg-orange-500 text-white border-none font-bold text-[10px] px-2 py-0.5 animate-pulse">
              LOW STOCK
            </Badge>
          </div>
        ) : null}
      ...
      <CardContent className="p-6 pb-2">
        ...
        <div className="mt-2.5 grid grid-cols-2 gap-2 text-xs border-t border-[#f1f5f9] pt-2">
          ...
          <div className="col-span-2">
            ...
            <span
              className="font-semibold text-zinc-700 line-clamp-1"
              title={product.suitable_vehicle}
            >
              {product.suitable_vehicle || "—"}
            </span>
          </div>
        </div>
        ...
        {brandOptions.length > 1 && (
          <div className="mt-4 pt-3 border-t border-[#f1f5f9]">
            <label className="text-[10px] font-bold text-[#94a3b8] uppercase tracking-widest mb-1.5 block">
              Select Brand Option
            </label>
            <select
            ...
            >
              {brandOptions.map((opt) => (
                <option key={opt.id} value={opt.id}>
                  {opt.brandName} - {currency}{" "}
                  {Number(
                    opt.is_on_offer ? opt.offer_price : opt.price
                  ).toLocaleString()}{" "}
                  {opt.is_on_offer ? "(On Offer)" : ""}
                </option>
              ))}
            </select>
          </div>
          ...
          <div className="flex gap-1.5">
            <span className="text-[10px] font-bold bg-[#eff6ff] text-[#0052cc] px-2 py-1 rounded uppercase tracking-wider">
              {activeProduct.weight
                ? `${Number(activeProduct.weight).toFixed(2)} KG`
                : "1.00 KG"}
            </span>
            <span
              className={cn(
                "text-[10px] font-black border px-2.5 py-1 rounded uppercase tracking-wider",
        ...
        <div className="mt-4 pt-4 border-t border-[#f1f5f9]">
          <label className="text-[10px] font-bold text-[#94a3b8] uppercase tracking-widest mb-2 block">
            Ship From Hub
          </label>
          <select
            ref={selectRef}
        ...
        <Button
          className={cn(
            ...
            activeProduct.status === "Inactive" || availableInventories.length === 0
              ...
              : isInCart
              ? "bg-[#ef4444] text-white hover:bg-[#dc2626] shadow-red-100"
          ...
          onClick={() => {
            ...
            if (isInCart) {
              ...
              const inv = availableInventories.find(
              ...
              if (inv) {
                ...
                toast.success(
                  `${product.name} (${activeProduct.brandName}) added to cart!`,
                  {
                    style: {
                      background: "#22c55e",
                      color: "#fff",
...
export function ProductsClient({
  ...
  // Local UI Filter States (initialized from URL query parameters)
  const [searchInput, setSearchInput] = useState(currentQueryParams.search || "");
  const [categorySearch, setCategorySearch] = useState("");
  const [brandSearch, setBrandSearch] = useState("");
  const [showMobileFilters, setShowMobileFilters] = useState(false);

  const selectedCategoryId = currentQueryParams.category_id
    ? Number(currentQueryParams.category_id)
    : null;
  const selectedBrandId = currentQueryParams.brand_id
    ? Number(currentQueryParams.brand_id)
    : null;
  const currentPage = currentQueryParams.page ? Number(currentQueryParams.page) : 1;
  const pageSize = currentQueryParams.per_page ? Number(currentQueryParams.per_page) : 9;

  // Function to sync filter changes to URL searchParams without triggering scroll jump
  const updateURL = useCallback(
    (updates: Record<string, string | number | null | undefined>) => {
      ...
      // scroll: false prevents scrolling back to the top of the page on filter change
      router.push(url, { scroll: false });
    },
    [router, pathname, searchParams]
  );

  // Debounced search input handler to update URL search param
  useEffect(() => {
    const timer = setTimeout(() => {
      if (searchInput !== (currentQueryParams.search || "")) {
        updateURL({ search: searchInput || null, page: 1 });
      }
    }, 400);
    return () => clearTimeout(timer);
  }, [searchInput, currentQueryParams.search, updateURL]);

  const toggleCategory = (catId: number) => {
    const nextVal = selectedCategoryId === catId ? null : catId;
    updateURL({ category_id: nextVal, page: 1 });
  };

  const toggleBrand = (bId: number) => {
    const nextVal = selectedBrandId === bId ? null : bId;
    updateURL({ brand_id: nextVal, page: 1 });
  };
  ...
  const handlePageSizeChange = (newSize: number) => {
    updateURL({ per_page: newSize, page: 1 });
  };
                      ...
                      <div className="relative">
                        ...
                        >
                          {categories.filter((cat) =>
                            ...
                            <p className="text-xs text-zinc-400 font-medium py-2 italic text-center">
                              No categories found
                            </p>
                          ) : (
                            categories
            ...
            <div id="tour-product-grid" className="flex-1">
              <div className="flex flex-col sm:flex-row justify-between items-center gap-4 mb-8 border-b border-[#f1f5f9] pb-6">
                ...
                <div className="w-full sm:w-auto text-right">
                  <Button
                    variant="ghost"
                    className="text-[13px] font-bold text-[#64748b] hover:bg-zinc-100/50 rounded-lg"
                  >
                    <Filter className="mr-2 h-4 w-4 text-zinc-500" /> Sort by:
                    Featured
                  </Button>
                </div>
              </div>
                ...
                <div className="space-y-8">
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    {initialProducts.map((product, index) => (
                      <div
                        key={product.id}
                        id={index === 0 ? "tour-product-card" : undefined}
                      >
